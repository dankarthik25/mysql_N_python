...
-- Q)  all books written before 1980
SELECT title, released_year FROM books WHERE released_year < 1980;

-- Q) all books written by Eggers Or Chabon
 SELECT title, author_lname FROM books WHERE author_lname='Eggers' OR author_lname='Chabon';

 --Q) all books written by Lahiri, Published after 2000
SELECT title, author_lname FROM books WHERE author_lname IN ('Eggers','Chabon');

-- Q) all books with page count btw 100 and 200
...
-- Q) all books where auther last name starts with 'C' or 'S'
SELECT
    title,
    author_lname
FROM books
...
-- Q) HOW many book author has written
